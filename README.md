...
#### 3. Найдём вероятность того, что ровно двум парам выпадут одинаковые игры
...
#### 4. Найдём вероятность того, что будет три и более пары участников играющих в одинаковые игры
```math
...
Где:
```math
p_{200} = \frac{200}{1600} = \frac{1}{8}, \quad p_{500} = \frac{500}{1600} = \frac{5}{16}.
```
...
Сложим вероятности того, что конкретный пункт выпадет ровно 7 раз, 8 раз, 9, ..., до 17:
```math
...
Просуммируем вероятности того, что ровно 0 пунктов выпадут 7 раз или более, ровно 1, ровно 2, ..., до 9:
```math
...
#### Вывод
Если участники будут крутить примерно по 200 колёс добра, а один из участников — значительно больше,
то вероятность того, что один пункт колеса выпадет хотя бы 7 участникам, близка к 100%.
А в трёх случаях из четырёх таких пунктов будет 10 и более.

## Заключение
Статистика [предыдущих](https://rgg.land/fall24/stats) [сезонов](https://rgg.land/stats) незначительно отличается от
приведённых теоретических расчётов. При схожих вводных получился схожий результат.
Это показывает, что рандом в RGG настоящий (случайный) или близкий к нему и имеет хорошее распределение.
Также это можно проверить в предстоящем RGG, если предварительно подготовить случайно перемешанные списки колёс добра
для каждого участника отдельно и убедиться, что это не повлияет на итоговую статистику колёс в конце сезона,
